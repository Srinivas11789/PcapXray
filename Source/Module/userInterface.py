...
class pcapXrayGui:
    def __init__(self, base):
        ...
        ttk.Label(
            InitFrame,
            text="Enter pcap file path: ",
            style="BW.TLabel").grid(
                column=0,
                row=0,
                sticky="W")
        self.filename_field = ttk.Entry(
            InitFrame,
            width=30,
            textvariable=self.pcap_file,
            style="BW.TEntry").grid(
                column=1,
                row=0,
                sticky="W, E")
        ...
        ttk.Button(
            InitFrame,
            text="Analyze!",
            command=self.pcap_analyse).grid(
                column=3,
                row=0,
                padx=10,
                pady=10,
        ...
        ttk.Button(
            SecondFrame,
            text="zoomOut",
            command=self.zoom_out).grid(
                row=10,
                column=11,
                sticky="E")

        # Third Frame with Results and Descriptioms
        self.ThirdFrame = ttk.Frame(
            base,
            width=100,
            height=100,
            padding="10 10 10 10",
    ...
    def browse_directory(self):
        ...
        self.pcap_file.set(
            tkFileDialog.askopenfilename(
                initialdir=sys.path[0], title="Select Packet Capture File!", filetypes=(
    ...
    def pcap_analyse(self):
        if os.path.exists(self.pcap_file.get()):
            ...
            result = Queue.Queue()
            packet_read = threading.Thread(
                target=pcapReader.pcapReader, args=(
                    self.pcap_file.get(), result))
            packet_read.start()
            ...
            reportThreadpcap = threading.Thread(
                target=reportGen.reportGen().packetDetails, args=(
                    self.capture_read,))
    ...
    def generate_graph(self):
        if self.name_servers == "":
            result = Queue.Queue()
            t = threading.Thread(
                target=communicationDetailsFetch.trafficDetailsFetch, args=(
                    self.capture_read, result))
            t.start()
            self.progressbar.start()
            while t.is_alive():
                self.progressbar.update()
            ...
            reportThread = threading.Thread(
                target=reportGen.reportGen().communicationDetailsReport, args=(
                    self.name_servers,))
            reportThread.start()

        if not os.path.exists(
                "Report/"+self.filename+self.option.get()+".png"):
            t1 = threading.Thread(
                target=plotLanNetwork.plotLan,
                args=(
                    self.capture_read,
                    self.filename,
                    self.name_servers,
